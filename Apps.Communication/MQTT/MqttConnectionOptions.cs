...
{
	/// <summary>
	/// 连接MQTT服务器的一些参数信息，适用<see cref="T:Communication.MQTT.MqttClient" />消息发布订阅客户端以及<see cref="T:Communication.MQTT.MqttSyncClient" />同步请求客户端。<br />
	/// Some parameter information for connecting to the MQTT server is applicable to the <see cref="T:Communication.MQTT.MqttClient" /> message publishing and subscription client and the <see cref="T:Communication.MQTT.MqttSyncClient" /> synchronization request client.
	/// </summary>
	public class MqttConnectionOptions
	{
		...
		/// <summary>
		/// 获取或设置当前的连接是否加密处理，防止第三方对注册报文进行抓包处理，从而分析出用户名和密码，只适用于基于Communication创建的MQTT Server。<br />
		...
		/// so as to analyze the user name and password. It is only applicable to the MQTT Server created based on Communication.
		/// </summary>
