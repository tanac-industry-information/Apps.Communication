...
namespace Apps.Communication.Algorithms.ConnectPool
{
	/// <summary>
	...
	public class ConnectPool<TConnector> where TConnector : IConnector
	{
		...
		/// <summary>
		/// 获取一个可用的连接对象，如果已经达到上限，就进行阻塞等待。当使用完连接对象的时候，需要调用<see cref="M:Communication.Algorithms.ConnectPool.ConnectPool`1.ReturnConnector(`0)" />方法归还连接对象。<br />
		...
		/// you need to call the <see cref="M:Communication.Algorithms.ConnectPool.ConnectPool`1.ReturnConnector(`0)" /> method to return the connection object.
		/// </summary>
