...
namespace Apps.Communication.Profinet.Siemens
{
	...
	public class SiemensMPI : SerialDeviceBase
	{
		...
		public OperateResult Handle()
		{
			while (true)
			{
				OperateResult<byte[]> operateResult = SPReceived(pipeSerial.GetPipe(), awaitData: true);
				...
				{
					OperateResult operateResult2 = SPSend(pipeSerial.GetPipe(), new byte[3] { 220, 0, 0 });
			...
			OperateResult operateResult3 = SPSend(pipeSerial.GetPipe(), new byte[3] { 220, 2, 0 });
		...
		public override OperateResult<byte[]> Read(string address, ushort length)
		{
			...
			OperateResult operateResult2 = SPSend(pipeSerial.GetPipe(), operateResult.Content);
			if (!operateResult2.IsSuccess)
			{
				return OperateResult.CreateFailedResult<byte[]>(operateResult2);
			}
			OperateResult<byte[]> operateResult3 = SPReceived(pipeSerial.GetPipe(), awaitData: true);
			if (!operateResult3.IsSuccess)
			{
				return OperateResult.CreateFailedResult<byte[]>(operateResult3);
			}
			...
			operateResult3 = SPReceived(pipeSerial.GetPipe(), awaitData: true);
			if (!operateResult3.IsSuccess)
			{
				return OperateResult.CreateFailedResult<byte[]>(operateResult3);
			}
			...
			operateResult2 = SPSend(pipeSerial.GetPipe(), readConfirm);
			if (!operateResult2.IsSuccess)
			{
				return OperateResult.CreateFailedResult<byte[]>(operateResult2);
			}
		...
		public override OperateResult Write(string address, byte[] value)
		{
			...
			operateResult2 = SPSend(pipeSerial.GetPipe(), writeConfirm);
			if (!operateResult2.IsSuccess)
			{
				return OperateResult.CreateFailedResult<byte[]>(operateResult2);
			}
