...
namespace Apps.Communication.Profinet.AllenBradley
{
	...
	public class AllenBradleyHelper
	{
		...
		/// <inheritdoc cref="M:Communication.Profinet.AllenBradley.AllenBradleyHelper.PackRequestHeader(System.UInt16,System.UInt32,System.UInt32,System.Byte[])" />
		...
		/// <returns>包含服务的信息</returns>
		public static byte[] PackCommandSingleService(byte[] command, ushort code = 178)
		{
			if (command == null)
			{
				command = new byte[0];
		...
		public static OperateResult<string> ReadPlcType(IReadWriteDevice plc)
		{
			...
			if (operateResult.Content.Length > 59)
			{
