...
{
	/// <summary>
	/// Communication的一些静态辅助方法<br />
	/// Some static auxiliary methods of Communication
	/// </summary>
	public class HslHelper
	{
		...
		/// <summary>
		/// 解析地址的附加<see cref="T:Communication.Core.DataFormat" />参数方法，比如你的地址是format=ABCD;D100，可以提取出"format"的值的同时，修改地址本身，如果"format"不存在的话，返回默认的<see cref="T:Communication.Core.IByteTransform" />对象<br />
		/// Parse the additional <see cref="T:Communication.Core.DataFormat" /> parameter method of the address. For example, if your address is format=ABCD;D100,
		/// you can extract the value of "format" and modify the address itself. If "format" does not exist, 
		/// Return the default <see cref="T:Communication.Core.IByteTransform" /> object
		/// </summary>
