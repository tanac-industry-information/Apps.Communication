...
namespace Apps.Communication
{
	...
	/// <br /><br />
	/// <note type="important">
	/// 本组件的目标是集成一个框架，统一所有的设备读写方法，抽象成统一的接口<see cref="T:Communication.Core.IReadWriteNet" />，对于上层操作只需要关注地址，读取类型即可，另一个目标是使用本框架轻松实现C#后台+C#客户端+web浏览器+android手机的全方位功能实现。
	/// </note>
	...
	/// <br />
	/// 关于Hsl的日志功能，贯穿整个hslcommunication的项目，所有的网络类，都包含了<see cref="T:Communication.LogNet.ILogNet" />日志功能，当然你也可以继承接口实现你自己的日志，在hsl里提供了三种常见的简单实用的日志
	...
	/// <note type="warning">
	/// 首先说明下<see cref="M:Communication.Core.Net.NetworkDoubleBase.ConnectServer" />方法里发生了什么？这个方法首先会关闭连接，然后重新连接，连接成功，就发送初始化指令（有些PLC就需要握手确认），初始化握手成功，才返回真正的成功！<br />
	/// 那么这里为什么不行呢？因为Read和Write方法是有混合锁实现互斥操作的，这样的好处就是多线程调用互不影响，但是<see cref="M:Communication.Core.Net.NetworkDoubleBase.ConnectServer" />方法，并没有互斥锁，如果调用的时候同时在读写，那就会导致异常，
	/// 那么为什么没有加互斥锁呢？因为为了实现读写方法的时候，支持自动重连操作，所以连接方法已经在互斥锁了。如果再加互斥锁，会发生死锁，所以综合考虑，就设计成了现在的样子。
	/// </note>
	/// 既然上面的代码不能使用，那么怎么来看当前的连接状态呢？这里有一点需要注意，<see cref="M:Communication.Core.Net.NetworkDoubleBase.ConnectServer" />只需要调用0次或1次即可。
	...
	/// <list type="bullet">
	///     <item>Hsl组件日志相关示例参考<see cref="T:Communication.LogNet.ILogNet" /></item>
	///     <item>三菱mc协议示例参考<see cref="T:Communication.Profinet.Melsec.MelsecMcNet" /></item>
	///     <item>西门子S7协议示例参考<see cref="T:Communication.Profinet.Siemens.SiemensS7Net" /></item>
	///     <item>欧姆龙协议示例参考<see cref="T:Communication.Profinet.Omron.OmronFinsNet" /></item>
	///     <item>罗克韦尔协议示例参考<see cref="T:Communication.Profinet.AllenBradley.AllenBradleyNet" /></item>
	///     <item>Modbus协议示例参考<see cref="T:Communication.ModBus.ModbusTcpNet" /></item>
	///     <item>MQTT服务器示例参考<see cref="T:Communication.MQTT.MqttServer" /></item>
	///     <item>MQTT客户端示例参考<see cref="T:Communication.MQTT.MqttClient" /></item>
	///     <item>WebSocket服务器示例参考<see cref="T:Communication.WebSocket.WebSocketServer" /></item>
	///     <item>WebSocket客户端示例参考<see cref="T:Communication.WebSocket.WebSocketClient" />。</item>
	///     <item>WebApi示例参考<see cref="T:Communication.Enthernet.HttpServer" />。</item>
	///     <item>Redis示例参考<see cref="T:Communication.Enthernet.Redis.RedisClient" /></item>
	///     <item>身份证阅读器串口版<see cref="T:Communication.Profinet.IDCard.SAMSerial" /></item>
	///     <item>身份证阅读器网口版<see cref="T:Communication.Profinet.IDCard.SAMTcpNet" /></item>
	///     <item>文件传送服务器<see cref="T:Communication.Enthernet.UltimateFileServer" /></item>
	/// </list>
	/// <note type="important">
	...
	/// <br />
	/// <list type="bullet">
	///     <item>企业授权提供专业版通讯库的所有更新版的 Communication 源代码。包含 .Net Java Python 三大平台。</item>
	...
	///
	/// 个人赞助二维码：<br />
	/// <img src="https://raw.githubusercontent.com/dathlin/Communication/master/imgs/support.png" />
	/// </remarks>
	/// <revisionHistory>
	...
	///             <item>将部分类从Communication命名空间下移动到Communication.Core下面。</item>
	///             <item>提供了一个通用的ModBus TCP的客户端类，方便和服务器交互。</item>
	///             <item>完善了Communication.BasicFramework.SoftBaisc下面的辅助用的静态方法，提供了一些方便的数据转化，在上面进行公开。</item>
	///         </list>
	///     </revision>
	...
	///             <item>Python：实现了python版本的Communication程序，基于pyqt实现，初步添加了一些PLC的调试界面。</item>
	///             <item>代码注释优化，使用前请仔细阅读下面的信息：http://api.hslcommunication.cn </item>
	///             <item>http://www.hslcommunication.cn/MesDemo 官网的地址以后作为优秀的MES产品展示平台，欢迎大家关注。</item>
	///             <item>HSL的目标是打造成工业互联网的利器，工业大数据的基础，打造边缘计算平台。</item>
	///         </list>
	...
	///             <item>UdpNet：添加ConnectionId属性，使用的<seealso cref="M:Communication.BasicFramework.SoftBasic.GetUniqueStringByGuidAndRandom" />方法获取信息。</item>
	...
	///             <item>Demo: Communication Test Demo支持PLC及一些连接对象的参数保存功能，使用英文冒号可以分类管理。</item>
	...
	///             <item>Communication: 所有的异步通信代码优化，优化超时检测机制，现在大大提升了服务器的高并发的能力，异步通信的性能。</item>
