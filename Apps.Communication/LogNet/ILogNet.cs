...
namespace Apps.Communication.LogNet
{
	...
	/// </remarks>
	/// <example>
	/// 自己实例化操作，在Communication里面，可选三种类型
	...
	public interface ILogNet : IDisposable
	{
		...
		/// <summary>
		/// 获取或设置当前的日志记录统计信息，如果你需要统计最近30天的日志，就需要对其实例化，详细参照<see cref="T:Communication.LogNet.LogStatistics" /><br />
		/// Get or set the current log record statistics. If you need to count the logs of the last 30 days, 
		/// you need to instantiate it. For details, please refer to <see cref="T:Communication.LogNet.LogStatistics" />
		/// </summary>
		...
		/// <summary>
		/// 过滤掉指定的关键字的日志，该信息不存储，但仍然触发<see cref="E:Communication.LogNet.ILogNet.BeforeSaveToFile" />事件<br />
		/// Filter out the logs of the specified keywords, the information is not stored, but the <see cref="E:Communication.LogNet.ILogNet.BeforeSaveToFile" /> event is still triggered
		/// </summary>
		/// <param name="keyword">关键字</param>
